...
Feature22::Feature22() {
	set_name("Fireplace");
	set_desc("There is a large fireplace in the library, but it doesn't appear to be lit. There's writing on the fireplace mantel.");
	set_index_id(21);
}
int Feature22::read(){
...
int Feature22::use(int obj_feat){
	if ((obj_feat==-1)&&(get_times_toggled(USE)==0)){
		...
		printf("The fireplace turns on");
		set_desc("There is a large fireplace in the library, and there's a regular fire crackling within. There's writing on the fireplace mantel.");
		
	}
	else if ((obj_feat==-1)&&(get_times_toggled(USE)>=1)){
		set_togg_count_x(USE, 0);
		printf("The fireplace turns off.");
		set_desc("There is a large fireplace in the library, but it doesn't appear to be lit. There's writing on the fireplace mantel.");
	}
	//if fire is on and you want to speak 
	else if ((obj_feat==DIARY)&&(get_times_toggled(USE)==1)){
		func_togg_count_x(USE);
		printf("The fire shudders and crackles, and briefly turns purple; the diary grows hot it in your hand, so hot you almost drop it.\n Then it flies open and reveals a spell to your eyes.\n You read it: 'Item ut in vicem dicere vessle fert ignis'");
