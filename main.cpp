...
//	explore_rooms.start();
	...
	//for (int i = 0; i < NUM_ROOMS; i++){
		//explore_rooms.get_player()->set_current_room();
		//printf("---------------ROOM %s-------------\n", explore_rooms.get_room_x(i)->get_name().c_str());
		for (int j = 0; j < RUN_FUNC_VERBS; j++){
			
			//get room id
			//run both features with all the verbs
			int room_id_zero = explore_rooms.get_room_x(0)->get_feature_x(0)->get_index_id();
			int room_id_one = explore_rooms.get_room_x(0)->get_feature_x(1)->get_index_id();
